...
public class Expirement extends java.lang.Object {
	...
	public static void main(java.lang.String[] args) {
		...
		for (i = 0; i < 500000; i++) {
			...
			bestRoad = road.fitness(individual) > bestRoad ? road.fitness(individual) : bestRoad;
			bestOnes = ones.fitness(individual) > bestOnes ? ones.fitness(individual) : bestOnes;
		...
		for (i = 1; i < 500000; i++) {
			if (!ones.isOptimal(hillOnes.getBest())) {
				hillOnes.step();
				bestOnes = ones.fitness(hillOnes.getBest()) > bestOnes ? ones.fitness(hillOnes
						.getBest()) : bestOnes;
				onesGenerated = i +1;
			}
			if (!road.isOptimal(hillRoad.getBest())) {
				hillRoad.step();
				bestRoad = road.fitness(hillRoad.getBest()) > bestRoad ? road.fitness(hillRoad
						.getBest()) : bestRoad;
				roadGenerated = i + 1;
			}
		}
		...
		System.out.println("	Best Royal Road fitness: " + bestRoad);
		System.out.println("    Royal Road final average fitness: " + popRoad.averageFitness());
	}
